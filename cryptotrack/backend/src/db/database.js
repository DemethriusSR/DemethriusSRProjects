...
/* ────────────────────────────────────────────────
 * MIGRAÇÃO — bancos já existentes (criados antes da
 * coluna role/active existir) precisam de ALTER TABLE,
 * já que CREATE TABLE IF NOT EXISTS não altera tabelas
 * que já existem.
 * ──────────────────────────────────────────────── */
...
if (!userColumns.includes('role')) {
  ...
  console.log('🔧 Migração: coluna role adicionada em users');
}

if (!userColumns.includes('active')) {
  db.exec(`ALTER TABLE users ADD COLUMN active INTEGER NOT NULL DEFAULT 1`);
  console.log('🔧 Migração: coluna active adicionada em users');
}
